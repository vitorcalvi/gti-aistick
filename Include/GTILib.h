...
struct GtiModelProperties{
	char name[FILENAME_STRING_SIZE]; // a null terminated string that holds a name string
...
public:
	...
	GtiTensor *next;   //GTI reserved for future use.
...
extern "C"
{
	...
	/*!
		\fn GTISDK_API int GtiGetModelInforFromModelFile(const char *modelFile, GtiModelInfor * modelInfor);
		\brief Get information of a model, including its designated device and input and output properties.
		\param[in] modelFile -- A constant char string containing the name of the model file
		\parma[out] modelInfor -- Information of this model
		\return -- Zero if the model information is not available; non-zero otherwise
	*/
	GTISDK_API int GtiGetInforFromModelFile(const char *modelFile, GtiModelInfor *modelInfor);


	/*!
		\fn GTISDK_API int GtiGetInforFromModelBuffer(const void *modelBuffer, int modelSize, GtiModelInfor * modelInfor);
		\brief Get information of a model, including its designated device and input and output properties.
		\param[in] modelBuffer -- A constant buffer containing a complete model
		\param[in] modelSize -- An integer containing the model's size in bytes
		\parma[out] modelInfor -- Information of this model
		\return -- Zero if the model information is not available; non-zero otherwise
	*/
	GTISDK_API int GtiGetInforFromModelBuffer(const void *modelBuffer, int modelSize, GtiModelInfor *modelInfor);
	...
	*/
	...
	/*!
		\fn GTISDK_API const char * GtiImageEvaluate(GtiModel *model, const char *image, int height, int width, int depth);
	...
	*/
	...
	/*!
		...
		\param[in] device -- A pointer to the GtiDevice
		\return -- A null terminated string representing the platform name
	*/

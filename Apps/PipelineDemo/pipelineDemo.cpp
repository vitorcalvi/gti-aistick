...
#ifdef WIN32
#include <windows.h>
unsigned long GetTickCount64_us() { return GetTickCount64() * 1000; }
#endif
...
vector<string> glob(const std::string &folder) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            if (wstring(fd.cFileName).compare(L".") == 0)
                continue;
            if (wstring(fd.cFileName).compare(L"..") == 0)
                continue;
...
void printResult(GtiTensor *tensorOut) {
    if ((tensorOut->format == TENSOR_FORMAT_BINARY) || (tensorOut->format == TENSOR_FORMAT_BINARY_INTEGER) ||
...
void evaluateCb(GtiModel *model, const char *layer, GtiTensor *tensorOut) {
    ...
    if (tensorOut->format == TENSOR_FORMAT_JSON)
        printResult(tensorOut);
}

int main(int argc, const char *argv[]) {
    if (argc < 3) {
        LOG(ERROR) << "Usage: " << argv[0] << " "
                   << "model_file image_dir";
        return -1;
    }
    int loops = 1;
    if (argc >= 4)
    ...
    if (fileArray.empty()) {
        LOG(ERROR) << "could not find files under dir: " << argv[2];
        return -1;
    }
    size_t seq = 0;
    GtiModel *model = GtiCreateModel(argv[1]);
    if (0 == model) {
        LOG(ERROR) << "could not open model: " << argv[1];
        return -1;
    }
    ...
    for (auto &i : fileArray) {
        ...
        if (!fin.is_open()) {
            LOG(ERROR) << "could not open file " << i;
            continue;
        }
